...
1. **Epic Title**: User-centric, value-focused. **Slug-friendly**: Epic titles are used for spec directory names (e.g. `specs/epic-N-{slug}/`). Use concise, alphanumeric-friendly titles that convert cleanly to kebab-case (e.g. "Indicator system refactor", "Metrics cache"); avoid colons or long phrases that become unwieldy as directory names.
