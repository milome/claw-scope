...
outputFile: '{planning_artifacts}/{branch}/epics.md'
...
Use the **exact** header format below so that downstream tools (e.g. create-new-feature.ps1) can derive directory slugs from epics.md. Required: `## Epic {N}: {epic_title}` for each epic section and `### Story {N}.{M}: {story_title}` for each story.
...
1. **Story Title**: Clear, action-oriented. **Slug-friendly**: Story titles are used for spec directory names (e.g. `story-M-{slug}/`). Use concise titles that convert cleanly to kebab-case (e.g. "Implement base cache class", "User registration with email"); avoid colons or special characters that don't translate well to directory names.
