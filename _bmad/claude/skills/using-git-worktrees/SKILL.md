...
<!-- ═══════════════════════════════════════════════════════════════════
     ...
     业务语义基线：worktree 创建/切换逻辑、自适应策略、安全验证
     ═══════════════════════════════════════════════════════════════════ -->
...
<!-- ═══════════════════════════════════════════════════════════════════
     ...
     执行体映射：冲突解决审计 → Agent tool + .claude/agents/
     ═══════════════════════════════════════════════════════════════════ -->

### 冲突解决审计 — Claude Code CLI 执行体映射

冲突解决后的审计流程在 Claude Code CLI 中通过 Agent tool（`subagent_type: general-purpose`）调度，主 Agent 将 `.claude/agents/auditors/auditor-implement.md` 整段传入作为完整 prompt。
...
主 Agent 在调用审计子代理前，必须输出 **CLI Calling Summary**：
...
#### Fallback Strategy（4 层降级 — Architecture D3）

当主执行体不可用时，按以下顺序降级：

| 层级 | 执行体 | 说明 |
|------|--------|------|
| L1 | `.claude/agents/auditors/auditor-implement.md` via Agent tool | 主路径：专用审计 agent |
| L2 | OMC `code-reviewer` | 回退 1：oh-my-claudecode reviewer |
| L3 | `.claude/skills/bmad-code-reviewer-lifecycle/SKILL.md` | 回退 2：code-review skill 直接调用 |
| L4 | 主 Agent 直接执行审计 | 最终回退：无子代理 |

**Fallback 降级通知**（FR26）：降级时须向用户显示当前使用的执行体层级：
...
冲突解决后，必须通过 Agent tool（`subagent_type: general-purpose`）触发审计：
...
<!-- ═══════════════════════════════════════════════════════════════════
     ...
     仓库级扩展：BMAD-Speckit 整合、状态跟踪、Quick Reference
     ═══════════════════════════════════════════════════════════════════ -->
