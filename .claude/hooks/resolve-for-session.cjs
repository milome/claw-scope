...
function loadConfigFromFile(configPath) {
  const path3 = configPath || DEFAULT_CONFIG_PATH;
  if (!(0, import_node_fs.existsSync)(path3)) {
    return {};
  }
  try {
    const content = (0, import_node_fs.readFileSync)(path3, "utf8");
    return load(content);
  } catch (error) {
    console.error(`[bmad-config] \u8BFB\u53D6\u914D\u7F6E\u6587\u4EF6\u5931\u8D25: ${path3}`, error);
    return {};
  }
}
...
function mergeLanguagePolicyIntoProjectContext(root, languagePolicy) {
...
function stripCodeLikeSegments(input) {
  return input.replace(/```[\s\S]*?```/g, " ").replace(/`[^`]*`/g, " ").replace(/https?:\/\/\S+/g, " ").replace(/[A-Za-z]:\\[^\s]*/g, " ").replace(/\/[\w./-]+/g, " ").trim();
}
function detectExplicitLanguageInstruction(message) {
  let lastMatch = null;
...
function classifyUserMessageLanguage(message) {
...
function resolveAutoDetectedLanguage(messages, allowBilingualAutoMode) {
...
function resolveLanguagePolicy(input) {
  ...
  const detectedMessages = input.recentUserMessages.slice(-3).map((message) => classifyUserMessageLanguage(message));
...
async function main() {
  ...
  const projectRoot = path2.resolve(String(body.projectRoot || process.cwd()));
  const userMessage = String(body.userMessage ?? "");
  const recentMessages = Array.isArray(body.recentMessages) ? body.recentMessages.map((x) => String(x)) : [];
