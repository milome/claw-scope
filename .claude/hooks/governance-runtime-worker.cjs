...
var require_ref = __commonJS({
  "../../node_modules/ajv/dist/vocabularies/core/ref.js"(exports2) {
    ...
    function getValidate2(cxt, sch) {
      const { gen } = cxt;
      return sch.validate ? gen.scopeValue("validate", { ref: sch.validate }) : (0, codegen_1._)`${gen.scopeValue("wrapper", { ref: sch })}.validate`;
    }
    exports2.getValidate = getValidate2;
...
var require_limit = __commonJS({
  "../../node_modules/ajv-formats/dist/limit.js"(exports2) {
    ...
    var formatLimitPlugin = (ajv5) => {
      ajv5.addKeyword(exports2.formatLimitDefinition);
      return ajv5;
    };
...
var require_dist = __commonJS({
  "../../node_modules/ajv-formats/dist/index.js"(exports2, module2) {
    ...
    var formatsPlugin = (ajv5, opts = { keywords: true }) => {
      if (Array.isArray(opts)) {
        addFormats2(ajv5, opts, formats_1.fullFormats, fullName);
        return ajv5;
      }
      const [formats, exportName] = opts.mode === "fast" ? [formats_1.fastFormats, fastName] : [formats_1.fullFormats, fullName];
      const list = opts.formats || formats_1.formatNames;
      addFormats2(ajv5, list, formats, exportName);
      if (opts.keywords)
        (0, limit_1.default)(ajv5);
      return ajv5;
    };
    ...
    function addFormats2(ajv5, list, fs20, exportName) {
      var _a;
      var _b;
      (_a = (_b = ajv5.opts.code).formats) !== null && _a !== void 0 ? _a : _b.formats = (0, codegen_1._)`require("ajv-formats/dist/formats").${exportName}`;
      for (const f of list)
        ajv5.addFormat(f, fs20[f]);
    }
...
var require_deferred_gap_governance = __commonJS({
  "../../_bmad/runtime/hooks/deferred-gap-governance.cjs"(exports2, module2) {
    ...
    function resolveProjectPath(projectRoot, candidate) {
      ...
      return path19.isAbsolute(candidate) ? candidate : path19.join(projectRoot, candidate);
    }
    ...
    function readDeferredGapsFromReport2(reportPath) {
      const markdown = fs20.readFileSync(reportPath, "utf8");
    ...
    function reportSortValue(reportPath) {
      const dateToken = extractDateToken(path19.basename(reportPath));
      if (dateToken) {
      ...
      try {
        return fs20.statSync(reportPath).mtimeMs;
      } catch {
        return 0;
      }
    }
    function listReadinessReports(projectRoot) {
      const planningRoot = path19.join(projectRoot, "_bmad-output", "planning-artifacts");
      const found = [];
      function walk(dir) {
    ...
    function findPreviousReadinessReport(currentReportPath) {
      const directory = path19.dirname(currentReportPath);
      if (!fs20.existsSync(directory)) return null;
      const candidates = fs20.readdirSync(directory).filter((entry) => READINESS_REPORT_PATTERN.test(entry)).map((entry) => path19.join(directory, entry)).filter((fullPath) => path19.resolve(fullPath) !== path19.resolve(currentReportPath)).sort((left, right) => reportSortValue(right) - reportSortValue(left));
    ...
    function parseYamlModule() {
      const candidates = [
        "js-yaml",
        path19.join(process.cwd(), "node_modules", "js-yaml"),
        path19.join(process.cwd(), "..", "BMAD-Speckit-SDD-Flow", "node_modules", "js-yaml"),
        path19.join(__dirname, "..", "..", "..", "node_modules", "js-yaml"),
        path19.join(__dirname, "..", "..", "..", "..", "node_modules", "js-yaml")
      ];
    ...
    function resolveFeatureArtifactRoot(projectRoot, options = {}) {
      ...
      if (explicitArtifact) candidates.push(path19.dirname(explicitArtifact));
      if (explicitRegister) candidates.push(path19.dirname(explicitRegister));
      const unique5 = [];
      for (const candidate of candidates) {
    ...
    function resolveDeferredGapRegisterLocation(projectRoot, options = {}) {
      ...
      return {
        ...
        register_path: path19.join(featureRoot, REGISTER_FILE_NAME)
      };
    }
    function loadDeferredGapRegister(projectRoot, options = {}) {
      ...
      if (!fs20.existsSync(resolved.register_path)) {
        return {
          ...resolved,
          exists: false,
          document: null,
      ...
      const document = yaml.load(fs20.readFileSync(resolved.register_path, "utf8")) || {};
    ...
    function findFirstExistingFile(baseDir, candidates) {
      for (const fileName of candidates) {
        const fullPath = path19.join(baseDir, fileName);
        if (fs20.existsSync(fullPath)) return fullPath;
      }
      return null;
    }
    function findTasksArtifact(baseDir, explicitArtifactPath) {
      if (explicitArtifactPath && /^tasks/i.test(path19.basename(explicitArtifactPath))) {
        return explicitArtifactPath;
      }
      if (!fs20.existsSync(baseDir)) return null;
      const matches = fs20.readdirSync(baseDir).filter((entry) => /^tasks.*\.md$/i.test(entry)).map((entry) => path19.join(baseDir, entry)).sort((left, right) => left.localeCompare(right));
    ...
    function collectJourneyArtifacts(projectRoot, options = {}) {
      ...
      const tasksMarkdown = tasksPath && fs20.existsSync(tasksPath) ? fs20.readFileSync(tasksPath, "utf8") : "";
      ...
      const standalone = {
        ...
        closure_notes_dir: fs20.existsSync(path19.join(featureRoot, CLOSURE_NOTES_DIR)) ? path19.join(featureRoot, CLOSURE_NOTES_DIR) : null
      };
      const closureNoteFiles = standalone.closure_notes_dir ? fs20.readdirSync(standalone.closure_notes_dir).filter((entry) => /\.md$/i.test(entry)).map((entry) => path19.join(standalone.closure_notes_dir, entry)) : [];
      return {
        feature_root: featureRoot,
    ...
    function validateJourneyArtifactPrecedence(artifacts) {
      ...
      const compareRef = (declaredPath, actualPath, label) => {
        ...
        const declaredBase = path19.basename(normalizeSlashes(declaredPath)).toLowerCase();
        const actualBase = path19.basename(normalizeSlashes(actualPath)).toLowerCase();
    ...
    function readSprintDeferredGapPlan(projectRoot) {
      const sprintPath = path19.join(projectRoot, "_bmad-output", "implementation-artifacts", "sprint-status.yaml");
      if (!fs20.existsSync(sprintPath)) {
        return {
      ...
      const yaml = parseYamlModule();
      const doc = yaml.load(fs20.readFileSync(sprintPath, "utf8")) || {};
    ...
    function buildDeferredGapStatusItems(projectRoot) {
      ...
      for (const reportPath of reports) {
        const key = path19.dirname(reportPath);
      ...
      for (const reportGroup of grouped.values()) {
        ...
        for (let index = 1; index < ordered.length; index += 1) {
          ...
          for (const removed of comparison.removed_without_evidence) {
            ...
            alerts.push(`Gap ${removed.gap_id} was removed without resolution evidence between ${path19.basename(ordered[index - 1])} and ${path19.basename(ordered[index])}`);
          }
        }
...
var require_governance_stage_event_emitter = __commonJS({
  "../../_bmad/runtime/hooks/governance-stage-event-emitter.cjs"(exports2, module2) {
    "use strict";
    var fs20 = require("node:fs");
    var path19 = require("node:path");
    function queueDir(projectRoot) {
      return path19.join(projectRoot, "_bmad-output", "runtime", "governance", "queue");
    }
    function pendingEventDir(projectRoot) {
      return path19.join(queueDir(projectRoot), "pending-events");
    }
    function sanitizeEventToken(value) {
      return String(value || "").replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^-+|-+$/g, "");
    }
    function buildGovernanceStageRerunResultEvent2(input) {
      return {
        type: "governance-rerun-result",
        payload: {
          projectRoot: input.projectRoot,
          ...input.configPath ? { configPath: input.configPath } : {},
          ...Array.isArray(input.journeyContractHints) && input.journeyContractHints.length > 0 ? { journeyContractHints: input.journeyContractHints } : {},
    ...
    function persistGovernanceStageRerunResultEvent2(event) {
      ...
      const rerunGate = typeof runnerInput.rerunGate === "string" && runnerInput.rerunGate.trim() !== "" ? runnerInput.rerunGate : "unknown-gate";
      const attemptId = typeof runnerInput.attemptId === "string" && runnerInput.attemptId.trim() !== "" ? runnerInput.attemptId : "unknown-attempt";
      ...
      fs20.writeFileSync(filePath, JSON.stringify(event, null, 2) + "\n", "utf8");
      return filePath;
    }
    module2.exports = {
      buildGovernanceStageRerunResultEvent: buildGovernanceStageRerunResultEvent2,
      persistGovernanceStageRerunResultEvent: persistGovernanceStageRerunResultEvent2
    };
  }
});

// ../../_bmad/runtime/hooks/governance-runner-summary-presenter.cjs
var require_governance_runner_summary_presenter = __commonJS({
  "../../_bmad/runtime/hooks/governance-runner-summary-presenter.cjs"(exports2, module2) {
    "use strict";
    var {
      buildGovernanceLatestRawEventSectionLines,
      normalizeGovernanceRunnerSummaryLines
    ...
    function buildSemanticSkillFeaturesValue(semanticSkillFeatures) {
      ...
      const renderedFeatures = semanticSkillFeatures.map((feature) => {
        ...
        const interactionHints = Array.isArray(record.interactionHints) ? record.interactionHints.join("|") : "";
        const researchPolicyHints = Array.isArray(record.researchPolicyHints) ? record.researchPolicyHints.join("|") : "";
        const delegationHints = Array.isArray(record.delegationHints) ? record.delegationHints.join("|") : "";
        const constraintHints = Array.isArray(record.constraintHints) ? record.constraintHints.join("|") : "";
        const details = [
    ...
    function buildSemanticFeatureTopNValue(semanticFeatureTopN) {
      ...
      const rendered = dimensions.map((dimension) => {
        ...
        const compact = values.map((item) => {
          ...
          const provenance = Array.isArray(candidate.provenanceSkillIds) ? candidate.provenanceSkillIds.join("|") : "";
    ...
    function buildProviderRecommendationItemsValue(recommendationItems) {
      ...
      return recommendationItems.map((item) => {
        ...
        const filteredBecause = Array.isArray(record.filteredBecause) ? record.filteredBecause.join("|") || "(none)" : "(none)";
    ...
    function buildGovernanceStructuredMetadataSectionLines(input = {}) {
      ...
      const executorRouting = normalizedInput.executorRouting && typeof normalizedInput.executorRouting === "object" ? normalizedInput.executorRouting : (
        /** @type {GovernanceExecutorRoutingProjection} */
        {}
      );
      const prioritizedSignals = Array.isArray(executorRouting.prioritizedSignals) ? executorRouting.prioritizedSignals : [];
      const executionIntentCandidate = normalizedInput.executionIntentCandidate && typeof normalizedInput.executionIntentCandidate === "object" ? normalizedInput.executionIntentCandidate : {};
      const executionPlanDecision = normalizedInput.executionPlanDecision && typeof normalizedInput.executionPlanDecision === "object" ? normalizedInput.executionPlanDecision : {};
      const executionSkillChain = Array.isArray(executionPlanDecision.skillChain) ? executionPlanDecision.skillChain : Array.isArray(executionIntentCandidate.skillChain) ? executionIntentCandidate.skillChain : [];
      const providerRecommendedSkillChain = Array.isArray(
        executionPlanDecision.providerRecommendedSkillChain
      ) ? executionPlanDecision.providerRecommendedSkillChain : Array.isArray(executionIntentCandidate.providerRecommendedSkillChain) ? executionIntentCandidate.providerRecommendedSkillChain : [];
      const availableSkills = Array.isArray(executionPlanDecision.availableSkills) ? executionPlanDecision.availableSkills : Array.isArray(executionIntentCandidate.availableSkills) ? executionIntentCandidate.availableSkills : [];
      const matchedAvailableSkills = Array.isArray(executionPlanDecision.matchedAvailableSkills) ? executionPlanDecision.matchedAvailableSkills : Array.isArray(executionIntentCandidate.matchedAvailableSkills) ? executionIntentCandidate.matchedAvailableSkills : [];
      const missingSkills = Array.isArray(executionPlanDecision.missingSkills) ? executionPlanDecision.missingSkills : Array.isArray(executionIntentCandidate.missingSkills) ? executionIntentCandidate.missingSkills : [];
      const executionSubagentRoles = Array.isArray(executionPlanDecision.subagentRoles) ? executionPlanDecision.subagentRoles : Array.isArray(executionIntentCandidate.subagentRoles) ? executionIntentCandidate.subagentRoles : [];
      const providerRecommendedSubagentRoles = Array.isArray(
        executionPlanDecision.providerRecommendedSubagentRoles
      ) ? executionPlanDecision.providerRecommendedSubagentRoles : Array.isArray(executionIntentCandidate.providerRecommendedSubagentRoles) ? executionIntentCandidate.providerRecommendedSubagentRoles : [];
      const providerRecommendationItems = executionPlanDecision.providerRecommendationItems && typeof executionPlanDecision.providerRecommendationItems === "object" ? executionPlanDecision.providerRecommendationItems : executionIntentCandidate.providerRecommendationItems && typeof executionIntentCandidate.providerRecommendationItems === "object" ? executionIntentCandidate.providerRecommendationItems : { skills: [], subagentRoles: [] };
      ...
      const governanceBlockedBy = Array.isArray(executionPlanDecision.blockedByGovernance) ? executionPlanDecision.blockedByGovernance : [];
      const governanceConstraints = Array.isArray(executionPlanDecision.governanceConstraints) ? executionPlanDecision.governanceConstraints : [];
      const semanticSkillFeatures = Array.isArray(executionPlanDecision.semanticSkillFeatures) ? executionPlanDecision.semanticSkillFeatures : Array.isArray(executionIntentCandidate.semanticSkillFeatures) ? executionIntentCandidate.semanticSkillFeatures : [];
      const semanticFeatureTopN = executionPlanDecision.semanticFeatureTopN && typeof executionPlanDecision.semanticFeatureTopN === "object" ? executionPlanDecision.semanticFeatureTopN : executionIntentCandidate.semanticFeatureTopN && typeof executionIntentCandidate.semanticFeatureTopN === "object" ? executionIntentCandidate.semanticFeatureTopN : null;
      return [
        ...
        `- Current Attempt Number: ${normalizedInput.currentAttemptNumber !== void 0 && normalizedInput.currentAttemptNumber !== null ? normalizedInput.currentAttemptNumber : "(none)"}`,
        `- Next Attempt Number: ${normalizedInput.nextAttemptNumber !== void 0 && normalizedInput.nextAttemptNumber !== null ? normalizedInput.nextAttemptNumber : "(none)"}`,
        ...
        `- Interaction Mode: ${executionPlanDecision.interactionMode || executionIntentCandidate.interactionMode || "(none)"}`,
        `- Skill Availability Mode: ${executionPlanDecision.skillAvailabilityMode || executionIntentCandidate.skillAvailabilityMode || "(none)"}`,
        `- Research Policy: ${executionPlanDecision.researchPolicy || executionIntentCandidate.researchPolicy || "(none)"}`,
        `- Delegation Preference: ${executionPlanDecision.delegationPreference || executionIntentCandidate.delegationPreference || "(none)"}`,
    ...
    function buildGovernanceRunnerCliPresentation2(input = {}) {
      const normalizedInput = input ?? {};
    ...
    function printGovernanceRunnerCliPresentation(presentation, log) {
      const sink = typeof log === "function" ? log : console.log;
      const lines = presentation && Array.isArray(presentation.combinedLines) ? presentation.combinedLines : [];
      for (const line of lines) {
        sink(line);
      }
    }
    module2.exports = {
      buildGovernanceRunnerCliPresentation: buildGovernanceRunnerCliPresentation2,
...
var bmad_runtime_worker_exports = {};
__export(bmad_runtime_worker_exports, {
  governanceCurrentRunPath: () => governanceCurrentRunPath,
  processQueue: () => processQueue
});
module.exports = __toCommonJS(bmad_runtime_worker_exports);
var fs19 = __toESM(require("node:fs"));
var path18 = __toESM(require("node:path"));

// ../scoring/governance/write-rerun-history.ts
...
function resolveSchemaDir(options) {
  ...
  const candidates = [
    import_path.default.join(process.cwd(), "packages", "scoring", "schema"),
    import_path.default.join(process.cwd(), "node_modules", "@bmad-speckit", "schema"),
    import_path.default.join(process.cwd(), "node_modules", "bmad-speckit", "node_modules", "@bmad-speckit", "schema"),
    import_path.default.join(process.cwd(), "node_modules", "bmad-speckit-sdd-flow", "packages", "schema"),
    import_path.default.join(__dirname, "..", "..", "schema"),
    import_path.default.join(__dirname, "..", "..", "scoring", "schema"),
    import_path.default.join(__dirname, "..", "..", "..", "schema")
  ];
  for (const candidate of candidates) {
...
function resolveDataPath(dataPath) {
  ...
  return path2.isAbsolute(dataPath) ? dataPath : path2.resolve(process.cwd(), dataPath);
}
...
function loadAllRecords(dataPath) {
  ...
  for (const e of entries) {
    ...
    const full = path2.join(base, e.name);
    if (e.name.endsWith(".json") && !EXCLUDED_JSON.includes(e.name)) {
      try {
        const content = fs2.readFileSync(full, "utf-8");
        records.push(...parseRecords(content));
      } catch {
      }
    }
  }
  const jsonlPath = path2.join(base, "scores.jsonl");
  if (fs2.existsSync(jsonlPath)) {
    const lines = fs2.readFileSync(jsonlPath, "utf-8").split("\n").map((l) => l.trim()).filter((l) => l.length > 0);
    for (const line of lines) {
      try {
...
// ../scoring/writer/write-score.ts
var fs4 = __toESM(require("fs"));
var path4 = __toESM(require("path"));

// ../scoring/writer/validate.ts
...
function getValidate() {
  ...
  const schemaPath = path3.join(schemaDir, "run-score-schema.json");
  const schema2 = JSON.parse(fs3.readFileSync(schemaPath, "utf-8"));
  const ajv5 = new import_ajv.default();
  (0, import_ajv_formats.default)(ajv5);
  validateFn = ajv5.compile(schema2);
  return validateFn;
}
...
function writeSingleFile(record, dataPath) {
  ...
  const filePath = path4.join(dataPath, `${record.run_id}.json`);
  fs4.writeFileSync(filePath, JSON.stringify(record, null, 2), UTF8);
}
function appendJsonl(record, dataPath) {
  ensureDataDir(dataPath);
  const jsonlPath = path4.join(dataPath, "scores.jsonl");
  const line = JSON.stringify(record) + "\n";
  fs4.appendFileSync(jsonlPath, line, UTF8);
}
function getDataPath(options) {
  const p = options?.dataPath;
  if (p != null) {
    return path4.isAbsolute(p) ? p : path4.resolve(process.cwd(), p);
  }
  return getScoringDataPath();
}
...
function tryReadExistingSingleFileRecord(record, dataPath) {
  ...
  try {
    ...
    return parsed.stage === record.stage ? parsed : void 0;
  } catch {
    return void 0;
  }
}
function mergeWithExistingSingleFileRecord(record, dataPath) {
  ...
  return {
    ...
    ...record.artifact_root == null && existing.artifact_root != null ? { artifact_root: existing.artifact_root } : {},
    ...
    ...record.run_group_id == null && existing.run_group_id != null ? { run_group_id: existing.run_group_id } : {},
...
// ../scoring/governance/write-rerun-history.ts
var VALID_RUN_SCORE_STAGES = /* @__PURE__ */ new Set([
  "prd",
  "arch",
  "epics",
  "story",
...
function resolveScoringDataPath(projectRoot) {
  ...
  if (envPath && envPath.trim() !== "") {
    return path5.isAbsolute(envPath) ? envPath : path5.resolve(projectRoot, envPath);
  }
  const candidates = [
    path5.join(projectRoot, "packages", "scoring", "data"),
    path5.join(projectRoot, "scoring", "data"),
    path5.join(projectRoot, "_bmad-output", "scoring")
  ];
...
function mergeGovernanceRerunHistory2(existing, incoming) {
  const merged = /* @__PURE__ */ new Map();
  for (const item of existing ?? []) {
    if (item?.event_id) {
      merged.set(item.event_id, item);
...
function findTargetRecord(input) {
  ...
  const candidates = records.filter((record) => {
    ...
    if (input.epicStory) {
      ...
      return parsed != null && parsed.epicId === input.epicStory.epicId && parsed.storyId === input.epicStory.storyId;
    }
    return input.runGroupId != null && record.run_group_id === input.runGroupId || input.runGroupId != null && record.run_id === input.runGroupId;
  });
...
function writeGovernanceRerunHistory(input) {
  ...
  const entry = {
    ...
    ...normalizeExecutorRouting(input.executorRouting) != null ? { executor_routing: normalizeExecutorRouting(input.executorRouting) } : {},
    ...normalizeSummaryLines(input.remediationAuditTraceSummaryLines) != null ? { summary_lines: normalizeSummaryLines(input.remediationAuditTraceSummaryLines) } : {},
    ...normalizeSummaryLines(input.runnerSummaryLines) != null ? { runner_summary_lines: normalizeSummaryLines(input.runnerSummaryLines) } : {}
  };
  const nextRecord = {
    ...target ?? {
    ...
    ...input.runtimePolicy?.triggerStage != null && target?.trigger_stage == null ? { trigger_stage: input.runtimePolicy.triggerStage } : {},
    governance_rerun_history: mergeGovernanceRerunHistory2(target?.governance_rerun_history, [
      entry
    ])
  };
...
var ajv = new import_ajv2.default({ allErrors: true, strict: true, allowUnionTypes: true });
...
var validateModelGovernanceHintCandidate = ajv.compile(modelGovernanceHintCandidateSchema);
var validateFilteredModelGovernanceHints = ajv.compile(filteredModelGovernanceHintsSchema);
...
var ajv2 = new import_ajv3.default({ allErrors: true, strict: true, allowUnionTypes: true });
var aliasMapSchema = {
  type: "object",
  minProperties: 1,
  additionalProperties: {
...
var validatePromptRoutingRuleSet = ajv2.compile(promptRoutingRuleSetSchema);
var validatePromptRoutingHints = ajv2.compile(promptRoutingHintsSchema);
function assertValidPromptRoutingRuleSet(ruleSet) {
  if (!validatePromptRoutingRuleSet(ruleSet)) {
    throw new Error(
      `Invalid prompt routing rule set: ${ajv2.errorsText(validatePromptRoutingRuleSet.errors, {
        separator: "; "
      })}`
    );
  }
...
function assertValidPromptRoutingHints(hints) {
  if (!validatePromptRoutingHints(hints)) {
    throw new Error(
      `Invalid prompt routing hints: ${ajv2.errorsText(validatePromptRoutingHints.errors, {
        separator: "; "
      })}`
    );
  }
...
// ../../scripts/prompt-routing-hints.ts
var DEFAULT_PROMPT_ROUTING_RULES_PATH = path6.join(
  "_bmad",
  "bmm",
  "data",
...
function resolveRulesPath(projectRoot, rulesPath = DEFAULT_PROMPT_ROUTING_RULES_PATH) {
  return path6.isAbsolute(rulesPath) ? rulesPath : path6.join(projectRoot, rulesPath);
}
...
function loadPromptRoutingRules(projectRoot, rulesPath = DEFAULT_PROMPT_ROUTING_RULES_PATH) {
  ...
  const raw = fs6.readFileSync(absolutePath, "utf8");
...
var ajv3 = new import_ajv4.default({ allErrors: true, strict: true, allowUnionTypes: true });
...
var validateExecutionIntentCandidate = ajv3.compile(executionIntentCandidateSchema);
var validateExecutionPlanDecision = ajv3.compile(executionPlanDecisionSchema);
function assertValidExecutionIntentCandidate(value) {
  if (!validateExecutionIntentCandidate(value)) {
    throw new Error(
      `Invalid execution intent candidate: ${ajv3.errorsText(validateExecutionIntentCandidate.errors, {
        separator: "; "
      })}`
    );
  }
...
function assertValidExecutionPlanDecision(value) {
  if (!validateExecutionPlanDecision(value)) {
    throw new Error(
      `Invalid execution plan decision: ${ajv3.errorsText(validateExecutionPlanDecision.errors, {
        separator: "; "
      })}`
    );
  }
...
function rootExists(rootPath) {
  return rootPath.trim() !== "" && fs7.existsSync(rootPath) && fs7.statSync(rootPath).isDirectory();
}
function hostSkillDir(projectRoot, hostKind) {
  switch (hostKind) {
    case "cursor":
      return path7.join(projectRoot, ".cursor", "skills");
    case "claude":
      return path7.join(projectRoot, ".claude", "skills");
    case "codex":
      return path7.join(projectRoot, ".codex", "skills");
    case "generic":
    default:
      return null;
  }
...
function globalHostSkillDir(homeDir, hostKind) {
  switch (hostKind) {
    case "cursor":
      return path7.join(homeDir, ".cursor", "skills");
    case "claude":
      return path7.join(homeDir, ".claude", "skills");
    case "codex":
      return path7.join(homeDir, ".codex", "skills");
    case "generic":
    default:
      return null;
  }
...
function candidateRoots(input) {
  ...
  roots.push({
    rootPath: path7.join(input.projectRoot, ".agents", "skills"),
    source: "project-agents",
    priority: 90
  });
  roots.push({
    rootPath: path7.join(input.projectRoot, "skills"),
  ...
  roots.push({
    rootPath: path7.join(homeDir, ".agents", "skills"),
...
function hasSkillMarkdown(dirPath) {
  try {
    return fs7.readdirSync(dirPath).some((entry) => /^SKILL(\.[^.]+)?\.md$/iu.test(entry));
  } catch {
    return false;
  }
}
function resolveSkillMarkdownPath(dirPath) {
  try {
    const fileName = fs7.readdirSync(dirPath).find((entry) => /^SKILL(\.[^.]+)?\.md$/iu.test(entry));
    return fileName ? path7.join(dirPath, fileName) : null;
  } catch {
    return null;
  }
}
...
function readSkillMetadata(skillMarkdownPath) {
  try {
    const markdown = fs7.readFileSync(skillMarkdownPath, "utf8");
...
function collectSkillEntries(root) {
  return fs7.readdirSync(root.rootPath, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => {
    const skillDir = path7.join(root.rootPath, entry.name);
...
function governanceExecutionStoreDir(projectRoot) {
  return path9.join(projectRoot, "_bmad-output", "runtime", "governance", "executions");
}
function governanceExecutionLoopDir(projectRoot, loopStateId) {
  return path9.join(governanceExecutionStoreDir(projectRoot), sanitizeToken(loopStateId));
}
...
function writeGovernancePacketExecutionRecord(projectRoot, record) {
  ...
  fs9.writeFileSync(file, JSON.stringify(record, null, 2) + "\n", "utf8");
  return record;
}
...
function listGovernancePacketExecutionRecords(projectRoot, loopStateId) {
  ...
  const records = [];
  for (const loopDir of fs9.readdirSync(root)) {
    const fullLoopDir = path9.join(root, loopDir);
    if (!fs9.statSync(fullLoopDir).isDirectory()) {
      continue;
    }
    for (const file of fs9.readdirSync(fullLoopDir)) {
      if (!file.endsWith(".json")) {
        continue;
      }
      records.push(
        JSON.parse(
          fs9.readFileSync(path9.join(fullLoopDir, file), "utf8")
        )
      );
    }
  }
...
function findLatestActiveGovernancePacketExecutionRecord(projectRoot, loopStateId) {
  return listGovernancePacketExecutionRecords(projectRoot).filter(
    (record) => record.loopStateId === loopStateId && !["gate_passed", "escalated"].includes(record.status)
...
function resolveHostLaunchSpec(input, options = {}) {
  ...
  const envStartupTimeoutMs = parsePositiveNumber(env[envVarKey(hostKind, "STARTUP_TIMEOUT_MS")]) ?? options.startupTimeoutMs ?? null;
  ...
  if (hostKind === "claude") {
    return defaultClaudeLaunchSpec(input.projectRoot, envStartupTimeoutMs ?? void 0);
  }
  return null;
}
...
function spawnSyncJsonLaunch(spec, input, env) {
  const spawnResult = (0, import_node_child_process.spawnSync)(spec.command, spec.args, {
  ...
  if (spawnResult.error) {
    const code = typeof spawnResult.error.code === "string" ? spawnResult.error.code : "unknown";
    if (code === "ETIMEDOUT") {
  ...
  const metadata = {
    ...
    exitCode: spawnResult.status,
    stderr,
    ..."metadata" in parsed && parsed.metadata && typeof parsed.metadata === "object" ? parsed.metadata : {}
  };
...
async function spawnPacketPromptLaunch(spec, input, env) {
  const packetPrompt = fs10.readFileSync(input.packetPath, "utf8");
  const spawnOptions = {
    cwd: input.projectRoot,
    env: buildLaunchEnv(spec, input, env),
    detached: true,
  ...
  const child = (0, import_node_child_process.spawn)(spec.command, spec.args, spawnOptions);
  ...
  const launchResult = await new Promise((resolve9) => {
    let settled = false;
    const settle = (result) => {
      if (settled) {
        return;
...
function nowIso2(input) {
  return (input ?? /* @__PURE__ */ new Date()).toISOString();
}
function addSeconds(input, seconds) {
  return new Date(input.getTime() + seconds * 1e3).toISOString();
}
...
function buildLaunchInfo(outcome, metadata = {}) {
  return {
    ...
    metadata: {
      ..."metadata" in outcome && outcome.metadata && typeof outcome.metadata === "object" ? outcome.metadata : {},
      ...metadata
    }
  };
}
async function processPendingExecutionRecords(projectRoot, options = {}) {
  ...
  const adapter = options.adapter ?? createGovernanceHostDispatchAdapter({
    env: options.launchEnv,
    startupTimeoutMs: options.startupTimeoutMs
  });
  const now = options.now ?? /* @__PURE__ */ new Date();
  const leaseOwner = options.leaseOwner ?? `dispatch-worker-${process.pid}`;
  const records = listGovernancePacketExecutionRecords(projectRoot).filter(
    (record) => ["pending_dispatch", "retry_pending", "leased"].includes(record.status)
  );
  const updated = [];
  for (const record of records) {
    ...
    const leased = updateGovernancePacketExecutionRecord(
      ...
      (current) => ({
        ...
        history: [
          ...current.history,
          { at: nowIso2(now), kind: "dispatch-lease-acquired", note: leaseOwner }
        ]
      })
    );
    const observedAt = nowIso2(now);
    ...
    for (const hostKind of hostCandidates) {
      ...
      const outcome = await adapter.launch({
        ...
        packetPath,
        leaseOwner,
        timeoutMs: (options.timeoutMinutes ?? config.execution?.execution.timeoutMinutes ?? 30) * 60 * 1e3,
        projectRoot
      });
      ...
      if (outcome.kind === "accepted") {
        history.push({
          ...
          note: hostKind === leased.authoritativeHost ? outcome.reason ?? `accepted by ${hostKind}` : `fallback ${hostKind} accepted${outcome.reason ? `: ${outcome.reason}` : ""}`
        });
      ...
      history.push({
        ...
        note: hostKind === leased.authoritativeHost ? `${hostKind}: ${outcome.reason}` : `fallback ${hostKind}: ${outcome.reason}`
      });
      if (dispatchAttemptCount >= maxDispatchAttempts) {
        break;
      }
...
function main() {
  ...
  void processPendingExecutionRecords(projectRoot).then((records) => {
...
// ../../scripts/governance-packet-reconciler.ts
var fs11 = __toESM(require("node:fs"));
var path10 = __toESM(require("node:path"));
function nowIso3(now = /* @__PURE__ */ new Date()) {
...
function reconcileGovernanceExecutionRecords(projectRoot, now = /* @__PURE__ */ new Date()) {
  const config = readGovernanceRemediationConfig(projectRoot);
  ...
  const packetPaths = new Set(
    records.flatMap((record) => Object.values(record.packetPaths).filter(Boolean))
  );
  for (const record of records) {
    ...
    if (record.status === "running") {
      ...
      const timeoutMs = (config.execution?.execution.timeoutMinutes ?? 30) * 60 * 1e3;
    ...
    if (record.status === "awaiting_rerun_gate" && record.rerunGateSchedule?.scheduledAt && Date.parse(record.rerunGateSchedule.scheduledAt) + (config.execution?.execution.timeoutMinutes ?? 30) * 60 * 1e3 <= now.getTime()) {
      updateGovernancePacketExecutionRecord(projectRoot, record.loopStateId, record.attemptNumber, (current) => ({
        ...current,
        status: "retry_pending",
        history: [
  ...
  const orphanPacketPaths = listPacketFiles(path10.join(projectRoot, "_bmad-output")).filter(
    (file) => !packetPaths.has(file)
  );
  const orphanExecutionRecordIds = listGovernancePacketExecutionRecords(projectRoot).filter(
    (record) => Object.values(record.packetPaths).some((packetPath) => packetPath && !fs11.existsSync(packetPath))
  ).map((record) => record.executionId);
  fs11.mkdirSync(governanceExecutionStoreDir(projectRoot), { recursive: true });
  fs11.writeFileSync(
    path10.join(governanceExecutionStoreDir(projectRoot), "reconciliation-report.json"),
    JSON.stringify(
      {
        reconciledAt: nowIso3(now),
        updatedRecordIds,
        orphanPacketPaths,
...
function resolveDeferredGapContext(input) {
  ...
  if (fs12.existsSync(candidateReportPath)) {
...
function writeGovernanceRemediationArtifact(input) {
  ...
  const absoluteOutputPath = path11.isAbsolute(input.outputPath) ? input.outputPath : path11.join(input.projectRoot, input.outputPath);
  fs12.mkdirSync(path11.dirname(absoluteOutputPath), { recursive: true });
  fs12.writeFileSync(absoluteOutputPath, result.markdown, "utf8");
  return result;
}
...
function runtimeContextRegistryPath(root) {
  return path12.join(root, "_bmad-output", "runtime", "registry.json");
}
function readRuntimeContextRegistry(root) {
  const file = runtimeContextRegistryPath(root);
  const raw = fs13.readFileSync(file, "utf8");
...
function projectContextPath(root) {
  return path13.join(root, "_bmad-output", "runtime", "context", "project.json");
}
function resolveRuntimeContextPath(root, explicitPath) {
  const candidate = explicitPath?.trim();
  if (candidate) {
    return path13.isAbsolute(candidate) ? candidate : path13.resolve(root, candidate);
  }
...
function loadConfigFromFile(configPath) {
  const path19 = configPath || DEFAULT_CONFIG_PATH;
  if (!(0, import_node_fs.existsSync)(path19)) {
    return {};
  }
  try {
    const content = (0, import_node_fs.readFileSync)(path19, "utf8");
    return load(content);
  } catch (error) {
    console.error(`[bmad-config] \u8BFB\u53D6\u914D\u7F6E\u6587\u4EF6\u5931\u8D25: ${path19}`, error);
    return {};
  }
}
...
function loadTriggerConfig(configPath) {
  const base = configPath ?? path14.resolve(process.cwd(), "_bmad", "_config", "scoring-trigger-modes.yaml");
  const resolved = path14.isAbsolute(base) ? base : path14.resolve(process.cwd(), base);
...
var ajv4 = new import_ajv5.default({ allErrors: true, strict: true, allowUnionTypes: true });
...
function defaultConfigDir() {
  return path15.resolve(process.cwd(), "_bmad", "_config");
}
function resolvePath(override, name) {
  if (override) {
    return path15.isAbsolute(override) ? override : path15.resolve(process.cwd(), override);
  }
  return path15.join(defaultConfigDir(), name);
}
...
function mergeRuntimePolicyTemplate(base, templateId, templatesPath, cfg) {
  const content = fs16.readFileSync(templatesPath, "utf8");
...
function resolveAbsolutePath(projectRoot, targetPath) {
  return path16.isAbsolute(targetPath) ? targetPath : path16.join(projectRoot, targetPath);
}
function resolveGovernanceScoringDataPath(projectRoot) {
  const envPath = process.env.SCORING_DATA_PATH;
  if (envPath && envPath.trim() !== "") {
    return path16.isAbsolute(envPath) ? envPath : path16.resolve(projectRoot, envPath);
  }
  const candidates = [
    path16.join(projectRoot, "packages", "scoring", "data"),
    path16.join(projectRoot, "scoring", "data"),
    path16.join(projectRoot, "_bmad-output", "scoring")
  ];
  return candidates.find((candidate) => fs17.existsSync(candidate)) ?? candidates[0];
}
function nowIso4() {
  return (/* @__PURE__ */ new Date()).toISOString();
}
function parseEpicStoryFromRuntimeContext2(runtimeContext) {
  const dottedStory = runtimeContext?.storyId?.match(/^(\d+)\.(\d+)$/);
  if (dottedStory) {
    return {
      epicId: Number(dottedStory[1]),
...
function loadJourneyContractHintsForRuntime(projectRoot, runtimeContext) {
  ...
  const epicStory = parseEpicStoryFromRuntimeContext2(runtimeContext);
...
function writeGovernanceAttemptLoopState(projectRoot, state) {
  ...
  fs17.writeFileSync(file, JSON.stringify(state, null, 2) + "\n", "utf8");
}
...
function ingestRerunGateResult(state, result) {
  ...
  if (lastAttempt) {
    lastAttempt.rerunGateResult = {
      ...result,
      observedAt: result.observedAt ?? nowIso4()
    };
  }
  const normalized = {
    ...result,
...
function writeGovernanceExecutorPacket(artifactPath, packet) {
  ...
  fs17.mkdirSync(path16.dirname(file), { recursive: true });
  fs17.writeFileSync(file, renderGovernanceExecutorPacket(packet), "utf8");
  return file;
}
...
async function runGovernanceRemediation(input) {
  ...
  fs17.writeFileSync(absoluteOutputPath, artifactWithRunnerSummary.markdown, "utf8");
...
async function main4() {
  const argvTokens = process.argv.map((value) => path16.basename(String(value)).toLowerCase());
  ...
  if (jsonInputPath) {
    const payload = JSON.parse(fs17.readFileSync(path16.resolve(jsonInputPath), "utf8"));
  ...
  const projectRoot = argValue2(args, "--projectRoot") ? path16.resolve(argValue2(args, "--projectRoot")) : process.cwd();
...
function governanceQueueDir(projectRoot) {
  return path17.join(projectRoot, "_bmad-output", "runtime", "governance", "queue");
}
function governanceQueueBucketDir(projectRoot, bucket) {
  return path17.join(governanceQueueDir(projectRoot), bucket);
}
function governancePendingQueueFilePath(projectRoot, id) {
  return path17.join(governanceQueueBucketDir(projectRoot, "pending"), `${id}.json`);
}
function governanceProcessingQueueFilePath(projectRoot, id) {
  return path17.join(governanceQueueBucketDir(projectRoot, "processing"), `${id}.json`);
}
function governanceDoneQueueFilePath(projectRoot, id) {
  return path17.join(governanceQueueBucketDir(projectRoot, "done"), `${id}.json`);
}
function governanceFailedQueueFilePath(projectRoot, id) {
  return path17.join(governanceQueueBucketDir(projectRoot, "failed"), `${id}.json`);
}
function governanceCurrentRunPath(projectRoot) {
  return path17.join(projectRoot, "_bmad-output", "runtime", "governance", "current-run.json");
}
function ensureGovernanceQueueDirs(projectRoot) {
  for (const bucket of ["pending", "processing", "done", "failed"]) {
    fs18.mkdirSync(governanceQueueBucketDir(projectRoot, bucket), { recursive: true });
  }
}
...
function appendGovernanceCurrentRun(projectRoot, entry) {
  ...
  fs18.writeFileSync(file, JSON.stringify(current, null, 2) + "\n", "utf8");
}
...
function ingestGovernanceExecutionResult(projectRootOrInput, maybeInput) {
  ...
  const input = typeof projectRootOrInput === "string" ? maybeInput : projectRootOrInput;
  const config = readGovernanceRemediationConfig(projectRoot);
  ...
  return updateGovernancePacketExecutionRecord(
    ...
    (record) => {
      ...
      return {
        ...record,
        status: isSuccess ? "awaiting_rerun_gate" : failureCount >= maxFailures ? "escalated" : "retry_pending",
        leaseOwner: null,
        leaseAcquiredAt: null,
        leaseExpiresAt: null,
        ...
        rerunGateSchedule: isSuccess ? {
          ...
          scheduledAt: config.execution?.rerunGate.autoSchedule ? nowIso5() : null,
          observedAt: null,
          note: config.execution?.rerunGate.autoSchedule ? "rerun gate scheduled after successful execution result ingestion" : "rerun gate awaiting external scheduling"
...
function ingestGovernanceRerunGateResult(projectRootOrInput, maybeInput) {
  ...
  const input = typeof projectRootOrInput === "string" ? maybeInput : projectRootOrInput;
  const config = readGovernanceRemediationConfig(projectRoot);
  const target = typeof input.attemptNumber === "number" ? { loopStateId: input.loopStateId, attemptNumber: input.attemptNumber } : findLatestActiveGovernancePacketExecutionRecord(projectRoot, input.loopStateId);
  if (!target) {
    return null;
  }
  const attemptNumber = "attemptNumber" in target ? target.attemptNumber : target.attemptNumber;
    ...
    (record) => {
      ...
      return {
        ...
        rerunGateSchedule: {
          ...
          observedAt: input.rerunGateResult.observedAt ?? nowIso5(),
          note: input.rerunGateResult.summary ?? null
        },
        history: [
          ...record.history,
          {
            at: input.rerunGateResult.observedAt ?? nowIso5(),
...
function main5() {
  ...
  const result = payload.kind === "execution" ? ingestGovernanceExecutionResult(payload.projectRoot, payload) : ingestGovernanceRerunGateResult(payload.projectRoot, payload);
  process.stdout.write(JSON.stringify(result, null, 2));
}
main5();

// ../../scripts/bmad-runtime-worker.ts
var import_governance_stage_event_emitter = __toESM(require_governance_stage_event_emitter());
var {
  buildGovernanceRunnerCliPresentation
} = require_governance_runner_summary_presenter();
var LEGACY_QUEUE_DIR = path18.join(".claude", "state", "runtime", "queue");
function legacyCurrentRunPath(projectRoot) {
  return path18.join(projectRoot, ".claude", "state", "runtime", "current-run.json");
}
function ensureLegacyQueueDirs(projectRoot) {
  for (const bucket of ["pending", "processing", "done", "failed"]) {
    fs19.mkdirSync(path18.join(projectRoot, LEGACY_QUEUE_DIR, bucket), { recursive: true });
  }
}
function readQueueFiles(dir) {
  ...
  return fs19.readdirSync(dir).filter((file) => file.endsWith(".json")).sort((left, right) => left.localeCompare(right));
}
function appendLegacyCurrentRun(projectRoot, item) {
  const currentRunFile = legacyCurrentRunPath(projectRoot);
  const existing = fs19.existsSync(currentRunFile) ? JSON.parse(fs19.readFileSync(currentRunFile, "utf8")) : [];
  existing.push(item);
  fs19.mkdirSync(path18.dirname(currentRunFile), { recursive: true });
  fs19.writeFileSync(currentRunFile, JSON.stringify(existing, null, 2) + "\n", "utf8");
}
...
async function processLegacyQueue(projectRoot) {
  ...
  const pendingDir = path18.join(projectRoot, LEGACY_QUEUE_DIR, "pending");
  const processingDir = path18.join(projectRoot, LEGACY_QUEUE_DIR, "processing");
  const doneDir = path18.join(projectRoot, LEGACY_QUEUE_DIR, "done");
  const failedDir = path18.join(projectRoot, LEGACY_QUEUE_DIR, "failed");
  for (const file of readQueueFiles(pendingDir)) {
    const itemPath = path18.join(pendingDir, file);
    const processingPath = path18.join(processingDir, file);
    fs19.renameSync(itemPath, processingPath);
    try {
      const item = JSON.parse(fs19.readFileSync(processingPath, "utf8"));
      processLegacyEvent(projectRoot, item);
      fs19.renameSync(processingPath, path18.join(doneDir, file));
    } catch {
      fs19.renameSync(processingPath, path18.join(failedDir, file));
    }
  }
}
function deriveRerunDecisionFromPayload(payload) {
  ...
  const signals = [
    ...new Set(
      (payload.journeyContractHints ?? []).map((hint) => typeof hint?.signal === "string" ? hint.signal : null).filter((signal) => Boolean(signal && signal.trim()))
    )
  ].sort();
  if (signals.length > 0) {
    return {
...
function syncExecutionProjectionIntoCurrentRun(projectRoot, executionRecords) {
  ...
  const currentRun = readGovernanceCurrentRun(projectRoot);
  ...
  fs19.mkdirSync(path18.dirname(governanceCurrentRunPath(projectRoot)), { recursive: true });
  fs19.writeFileSync(
...
async function processGovernanceRerunEvent(queueProjectRoot, item) {
  ...
  if (config.execution?.enabled && typeof runnerInput.loopStateId === "string" && runnerInput.loopStateId && runnerInput.rerunGateResult) {
  ...
  writeGovernanceRerunHistory({
    ...
    loopStateId: result.loopState.loopStateId,
    runtimeContext: result.runtimeContext,
    runtimePolicy: result.runtimePolicy ? { triggerStage: result.runtimePolicy.triggerStage } : null,
    executorRouting,
  ...
  fs19.mkdirSync(path18.dirname(debugPath), { recursive: true });
  fs19.writeFileSync(
    debugPath,
    JSON.stringify(
      {
  ...
  if (item.type === "governance-pre-continue-check") {
    ...
    if (payload.status === "fail" && payload.rerunGate) {
      const remediationResult = await runGovernanceRemediation({
        ...
        outputPath: payload.artifactPath ?? path18.join(payload.projectRoot ?? queueProjectRoot, "_bmad-output", "planning-artifacts", "gate-remediation.md"),
    ...
    if (payload.status === "fail" && payload.rerunGate) {
      (0, import_governance_stage_event_emitter.persistGovernanceStageRerunResultEvent)(
        (0, import_governance_stage_event_emitter.buildGovernanceStageRerunResultEvent)({
          projectRoot: payload.projectRoot ?? queueProjectRoot,
          sourceEventType: "governance-pre-continue-check",
          runnerInput: {
            projectRoot: payload.projectRoot ?? queueProjectRoot,
            outputPath: payload.artifactPath ?? path18.join(
    ...
    const passthroughItem2 = {
      ...item,
      processedAt: (/* @__PURE__ */ new Date()).toISOString(),
      result
    };
    appendGovernanceCurrentRun(queueProjectRoot, passthroughItem2);
    return passthroughItem2;
  }
...
async function processGovernanceQueue(projectRoot) {
  ...
  const pendingDir = path18.dirname(governancePendingQueueFilePath(projectRoot, "queue-probe"));
  for (const file of readQueueFiles(pendingDir)) {
    const itemPath = path18.join(pendingDir, file);
    const itemId = path18.basename(file, ".json");
    const processingPath = governanceProcessingQueueFilePath(projectRoot, itemId);
    fs19.renameSync(itemPath, processingPath);
    try {
      const item = JSON.parse(
        fs19.readFileSync(processingPath, "utf8")
      );
      const finalizedItem = await processGovernanceEvent(projectRoot, item);
      fs19.writeFileSync(processingPath, JSON.stringify(finalizedItem, null, 2) + "\n", "utf8");
      fs19.renameSync(processingPath, governanceDoneQueueFilePath(projectRoot, itemId));
    } catch (error) {
      const failedDebugPath = path18.join(
        projectRoot,
        "_bmad-output",
        "runtime",
        "governance",
      ...
      try {
        ...
        failedItem.processedAt = (/* @__PURE__ */ new Date()).toISOString();
        failedItem.error = error instanceof Error ? error.message : String(error);
        fs19.writeFileSync(processingPath, JSON.stringify(failedItem, null, 2) + "\n", "utf8");
      } catch {
      }
      fs19.mkdirSync(path18.dirname(failedDebugPath), { recursive: true });
      fs19.writeFileSync(
        failedDebugPath,
        JSON.stringify(
          {
      ...
      fs19.renameSync(processingPath, governanceFailedQueueFilePath(projectRoot, itemId));
    }
  }
}
