...
var governance_packet_reconciler_exports = {};
__export(governance_packet_reconciler_exports, {
  reconcileGovernanceExecutionRecords: () => reconcileGovernanceExecutionRecords
});
module.exports = __toCommonJS(governance_packet_reconciler_exports);
...
function reconcileGovernanceExecutionRecords(projectRoot, now = /* @__PURE__ */ new Date()) {
  ...
  const orphanPacketPaths = listPacketFiles(path4.join(projectRoot, "_bmad-output")).filter(
    (file) => !packetPaths.has(file)
  );
  const orphanExecutionRecordIds = listGovernancePacketExecutionRecords(projectRoot).filter(
    (record) => Object.values(record.packetPaths).some((packetPath) => packetPath && !fs3.existsSync(packetPath))
  ).map((record) => record.executionId);
  fs3.mkdirSync(governanceExecutionStoreDir(projectRoot), { recursive: true });
  fs3.writeFileSync(
    path4.join(governanceExecutionStoreDir(projectRoot), "reconciliation-report.json"),
    JSON.stringify(
      {
        reconciledAt: nowIso2(now),
        updatedRecordIds,
        orphanPacketPaths,
...
0 && (module.exports = {
  reconcileGovernanceExecutionRecords
});
