...
var governance_packet_dispatch_worker_exports = {};
__export(governance_packet_dispatch_worker_exports, {
  StubGovernanceExecutionAdapter: () => StubGovernanceExecutionAdapter,
  createAcceptedPlaceholderDispatchAdapter: () => createAcceptedPlaceholderDispatchAdapter,
  processPendingExecutionRecords: () => processPendingExecutionRecords
});
module.exports = __toCommonJS(governance_packet_dispatch_worker_exports);
...
function governanceRemediationConfigPath(projectRoot) {
  return path2.join(projectRoot, "_bmad", "_config", "governance-remediation.yaml");
}
...
function readGovernanceRemediationConfig(projectRoot, explicitPath) {
  const file = explicitPath ? path2.isAbsolute(explicitPath) ? explicitPath : path2.resolve(projectRoot, explicitPath) : governanceRemediationConfigPath(projectRoot);
...
function governanceExecutionStoreDir(projectRoot) {
  return path3.join(projectRoot, "_bmad-output", "runtime", "governance", "executions");
}
function governanceExecutionLoopDir(projectRoot, loopStateId) {
  return path3.join(governanceExecutionStoreDir(projectRoot), sanitizeToken(loopStateId));
}
...
function listGovernancePacketExecutionRecords(projectRoot, loopStateId) {
  ...
  const records = [];
  for (const loopDir of fs2.readdirSync(root)) {
    const fullLoopDir = path3.join(root, loopDir);
    if (!fs2.statSync(fullLoopDir).isDirectory()) {
      continue;
    }
    for (const file of fs2.readdirSync(fullLoopDir)) {
      if (!file.endsWith(".json")) {
        continue;
      }
      records.push(
        JSON.parse(
          fs2.readFileSync(path3.join(fullLoopDir, file), "utf8")
        )
      );
    }
  }
...
async function spawnPacketPromptLaunch(spec, input, env) {
  const packetPrompt = fs3.readFileSync(input.packetPath, "utf8");
  const spawnOptions = {
    cwd: input.projectRoot,
    env: buildLaunchEnv(spec, input, env),
    detached: true,
  ...
  const launchResult = await new Promise((resolve2) => {
    let settled = false;
    const settle = (result) => {
      if (settled) {
        return;
...
// ../../scripts/governance-packet-dispatch-worker.ts
var StubGovernanceExecutionAdapter = class {
  constructor(outcome) {
...
0 && (module.exports = {
  StubGovernanceExecutionAdapter,
