...
function loadConfigFromFile(configPath) {
  const path6 = configPath || DEFAULT_CONFIG_PATH;
  if (!(0, import_node_fs.existsSync)(path6)) {
    return {};
  }
  try {
    const content = (0, import_node_fs.readFileSync)(path6, "utf8");
    return load(content);
  } catch (error) {
    console.error(`[bmad-config] \u8BFB\u53D6\u914D\u7F6E\u6587\u4EF6\u5931\u8D25: ${path6}`, error);
    return {};
  }
}
...
function defaultConfigDir() {
  return path2.resolve(process.cwd(), "_bmad", "_config");
}
function resolvePath(override, name) {
  if (override) {
    return path2.isAbsolute(override) ? override : path2.resolve(process.cwd(), override);
  }
  return path2.join(defaultConfigDir(), name);
}
...
function mergeRuntimePolicyTemplate(base, templateId, templatesPath, cfg) {
  const content = fs2.readFileSync(templatesPath, "utf8");
...
function runtimeContextRegistryPath(root) {
  return path3.join(root, "_bmad-output", "runtime", "registry.json");
}
...
function projectContextPath(root) {
  return path4.join(root, "_bmad-output", "runtime", "context", "project.json");
}
function resolveRuntimeContextPath(root, explicitPath) {
  const candidate = explicitPath?.trim();
  if (candidate) {
    return path4.isAbsolute(candidate) ? candidate : path4.resolve(root, candidate);
  }
...
function pickRoot(args) {
  ...
  if (fromArg) return path5.resolve(fromArg);
  return process.cwd();
}
function mainEmitRuntimePolicy(argv) {
  ...
  let needChdir = false;
  if (path5.resolve(prevCwd) !== path5.resolve(root)) {
    process.chdir(root);
    needChdir = true;
  }
  try {
