...
// ../../scripts/i18n/render-audit-block-cli.ts
var import_node_fs2 = require("node:fs");
var path2 = __toESM(require("node:path"));

// ../../scripts/i18n/load-manifest.ts
...
function validateTemplateManifest(manifest) {
  ...
  for (const [key, variants] of Object.entries(manifest.strings ?? {})) {
    const providedValues = Object.values(variants).filter(
      (value) => Boolean(value)
    );
...
function readResolvedMode() {
  const ctxPath = path2.join(process.cwd(), "_bmad-output", "runtime", "context", "project.json");
  if (!(0, import_node_fs2.existsSync)(ctxPath)) return "en";
  try {
    const raw = JSON.parse((0, import_node_fs2.readFileSync)(ctxPath, "utf8"));
