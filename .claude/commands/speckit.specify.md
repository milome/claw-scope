...
3. **Search for related design documents** (before generating spec):
   ...
   a. Run the script `_bmad/speckit/scripts/powershell/find-related-docs.ps1 -Json` to find existing design documents:
      ...
      - Search locations:
        ...
        - `specs/TBD - */*.md` - TBD directories with design documents
        - `poc/*/*.md` - POC directories with related documents
        - `docs/*.md` - Documentation files
      - The script uses keyword matching and relevance scoring
      - Returns top 10 most relevant documents
   
   b. Parse the JSON output to get list of related documents with their paths and relevance scores
   
   c. If related documents found:
      - Read the content of top 3-5 most relevant documents (relevance score > 10)
      - Extract key information: design decisions, architecture, requirements, constraints
      - Note: These documents will be referenced in the spec.md as "Reference Documents" section
   
   d. **IMPORTANT**: Use information from these documents to inform spec generation, but:
      - Do NOT copy implementation details into spec.md (spec should remain technology-agnostic)
...
6. Write the specification to SPEC_FILE using the template structure, replacing placeholders with concrete details derived from:
   - The feature description (arguments)
   - Information extracted from related design documents (if found)
   - Preserve section order and headings
   
   **Add Reference Documents section** (if related documents were found):
   - Add a new section at the end of spec.md: "## Reference Documents"
   - List all related documents found, with their relative paths
   - Format: `- [Document Name](relative/path/to/document.md) - [Brief description of relevance]`
   - Example:
     ```markdown
     ## Reference Documents
...
8. Report completion with:
   ...
   - Spec file path
   - Checklist results
   - List of related documents found and referenced (if any)
