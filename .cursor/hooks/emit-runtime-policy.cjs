...
function resolveRuntimePolicy(input) {
  ...
  const compatibilitySource = process.env.BMAD_RUNTIME_SHADOW === "1" ? "shadow" : "governance";
  let policy = {
    flow,
    stage,
    auditRequired,
...
function loadContextFromRegistry(root) {
...
function mainEmitRuntimePolicy(argv) {
  ...
  try {
    ...
    const contextProvided = Boolean(process.env.BMAD_RUNTIME_RUN_ID && process.env.BMAD_RUNTIME_RUN_ID.trim()) || Boolean(process.env.BMAD_RUNTIME_STORY_ID && process.env.BMAD_RUNTIME_STORY_ID.trim());
    ...
    const input = {
      ...
      ...process.env.BMAD_RUNTIME_STORY_SLUG ? { storySlug: process.env.BMAD_RUNTIME_STORY_SLUG } : {},
      ...process.env.BMAD_RUNTIME_RUN_ID ? { runId: process.env.BMAD_RUNTIME_RUN_ID } : {},
      ...process.env.BMAD_RUNTIME_ARTIFACT_ROOT ? { artifactRoot: process.env.BMAD_RUNTIME_ARTIFACT_ROOT } : {}
    };
    if (templateId) {
