                ...
                <div key={group.id} className="relative pl-6 rtl:pl-0 rtl:pr-6 md:pl-10 rtl:md:pr-10">
                  ...
                  <div className={`mb-4 rounded-2xl border p-4 transition md:mb-5 ${group.entries.some((entry) => entry.name === selectedTimelineEntryName) ? "border-sky-300 bg-sky-50 shadow-sm dark:border-sky-700 dark:bg-slate-800" : "border-slate-200 bg-slate-50/70 dark:border-slate-800 dark:bg-slate-900/70"}`}>
                    <div className="flex flex-wrap items-center gap-3 pt-0.5 md:pt-1">
                      ...
                      <span className="rounded-full border border-slate-200/80 bg-slate-200/60 px-2 py-0.5 text-[10px] font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 md:text-[11px]">
                        {t("memory.footprints.entries", group.entries.length)}
                      </span>
                      {group.probeDay && (
                        <span className="rounded-full border border-sky-100 bg-sky-50 px-2 py-0.5 text-[10px] font-semibold text-sky-700 dark:border-sky-800/50 dark:bg-sky-900/30 dark:text-sky-300 md:text-[11px]">
                        ...
                        <div className={`cursor-pointer rounded-xl border p-3.5 shadow-sm transition-all group-focus:ring-2 group-focus:ring-sky-500 md:rounded-lg md:p-4 md:hover:border-sky-300 dark:md:hover:border-sky-700 ${selectedTimelineEntryName === item.name ? "border-sky-300 bg-sky-50 dark:border-sky-700 dark:bg-slate-800" : "border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"}`}>
                          <div className="mb-2 flex items-center justify-between">
                            <div className="flex items-center gap-1.5 md:gap-2">
                              <span className="flex items-center gap-1 rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] font-mono text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 md:text-[11px]" dir="ltr">
                                <Clock className="h-2.5 w-2.5 md:h-3 md:w-3" /> {item.updatedAtMs ? new Date(item.updatedAtMs).toLocaleTimeString() : "-"}
                              </span>
                              <span className="flex items-center gap-1 rounded border border-cyan-100 bg-cyan-50 px-1.5 py-0.5 text-[10px] font-medium text-cyan-700 dark:border-cyan-800/50 dark:bg-cyan-900/20 dark:text-cyan-300">
                                <Network className="h-2.5 w-2.5 md:h-3 md:w-3" />
                                <span className="max-w-[120px] truncate md:max-w-none">{timelineAccess?.mode ?? timelineResult?.source ?? t("memory.footprints.timelineFallback")}</span>
                              </span>
                              {getAgentBadge(selectedAgentId)}
                            </div>
                            <span className="rounded border border-sky-100 bg-sky-50 px-1.5 py-0.5 text-[10px] font-medium text-sky-700 dark:border-sky-800/50 dark:bg-sky-900/30 dark:text-sky-300 md:px-2 md:text-[11px]">
                              {item.name}
                            </span>
                          </div>
